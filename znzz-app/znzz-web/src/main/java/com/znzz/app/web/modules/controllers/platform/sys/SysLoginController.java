...
import com.znzz.app.web.commons.shiro.filter.PlatformAuthenticationFilter;
import com.znzz.app.web.commons.slog.SLogService;
import com.znzz.framework.base.Result;
import com.znzz.framework.shiro.exception.CaptchaEmptyException;
import com.znzz.framework.shiro.exception.CaptchaIncorrectException;
...
public class SysLoginController {
    ...
    public Object doLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        ...
        try {
            ...
            List<String> data = userService.getUserPermissionData(user.getId()) ;
            user.setUserPermissionData(data) ;
            ...
            if(roles!=null){
                ...
                List<Sys_menu> allmenus = user.getMenus();//获取所有mennu
                List<Sys_menu> userFastIntoMenu = userService.getUserFastIntoMenu(allmenus,StringUtils.strip(builder.toString(), ","));
                Sys_menu sysMenu = userService.getLoginMenu(StringUtils.strip(builder.toString(), ","));
                if(sysMenu!=null){
                    href = sysMenu.getHref() ;
                }

                for(int i=0;i<userFastIntoMenu.size();i++){
                    StringBuilder build = new StringBuilder();
                    Sys_menu mm = userFastIntoMenu.get(i);
                    Sys_menu remm = convertToParentAndChild(allmenus, mm.getId());
                    mm.setName(remm.getName());
                    mm.setPermission(remm.getPermission());

                }

                user.setUserFastIntoMenu(userFastIntoMenu);
            }




            return Result.success("login.success",href);
    ...
    public Sys_menu convertToParentAndChild(List<Sys_menu> allMenu,String id){
        Sys_menu re= null ;
        for(int i=0;i<allMenu.size();i++){
            Sys_menu sys_menu = allMenu.get(i);
            if(sys_menu.getId().equals(id)){
                if(StringUtils.isNotBlank(sys_menu.getParentId())){
                    re = convertToParentAndChild(allMenu,sys_menu.getParentId()) ;
                }else{
                    re =  sys_menu ;
                }
            }
