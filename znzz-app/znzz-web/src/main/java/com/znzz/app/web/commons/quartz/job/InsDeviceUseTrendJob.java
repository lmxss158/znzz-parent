 ...
 */
@IocBean
public class InsDeviceUseTrendJob implements Job{
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		//获取单位开机数
		Map<String, Integer> poMap = homeService.getLastHourPowerOnNum() ;
		//获取单位今日历史
		Map<String, Ins_DeviceUseTrend> uhmap = homeService.getTodayUnitUseTrend(null) ;
		//插入list
		List<Ins_DeviceUseTrend> insertList = new ArrayList<Ins_DeviceUseTrend>() ;
		//更新list
		List<Ins_DeviceUseTrend> updateList = new ArrayList<Ins_DeviceUseTrend>() ;
		...
		while(it.hasNext()){
			...
			Ins_DeviceUseTrend trend = null ;
			Map<String, String> map = null ;
			if(uhmap.containsKey(unitId)){//包含子级
				trend = uhmap.get(unitId) ;
				String data = trend.getHistoryData() ;
				map = Json.fromJsonAsMap(String.class, data) ;
			}else{
				trend = new Ins_DeviceUseTrend() ;
				trend.setDate(new Date());
				trend.setUnitId(unitId);
				map = new HashMap<String,String>() ;
			}
			
			map.put(hour, totalPoNum+"") ;
			trend.setHistoryData(Json.toJson(map));
			
			if(null==trend.getId()){
				insertList.add(trend) ;
			}else{
				updateList.add(trend) ;
			}
			
		}
		homeService.insertListUseTrend(insertList);
		homeService.updateListUseTrend(updateList); 
		log.info("~~~~~~~~~~24小时设备使用趋势图结束~~~~~~~~~~");
