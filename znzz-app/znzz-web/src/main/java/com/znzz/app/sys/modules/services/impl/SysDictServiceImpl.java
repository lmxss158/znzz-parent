...
public class SysDictServiceImpl extends BaseServiceImpl<Sys_dict> implements SysDictService {
	...
	@Override
	public String getDictFromRedis(String key) {
		String dictJson = redisService.get(Globals.DICTKEY+key);
		if(dictJson==null || "".equals(dictJson)){
	        ...
	        for (Sys_dict dict : dictList) {
	        	  List<Sys_dict> list = getSubListByCode(dict.getCode());
	        	  redisService.set(Globals.DICTKEY+dict.getCode(), Json.toJson(list));
	        }
	        dictJson = redisService.get(Globals.DICTKEY+key);
		}
		return dictJson;
	}
}
